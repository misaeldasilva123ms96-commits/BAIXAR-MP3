...
function isDesktop() { return location.hostname === '127.0.0.1' || location.hostname === 'localhost'; }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      for (const job of active) {
        try { const next = await provider.getProgress(job.id); setJobs((current) => current.map((item) => item.id === next.id ? next : item)); } catch { /* transient polling failure is shown by the existing state */ }
      }
    }, 1000);
  ...
  return <div className="app-shell">
    ...
    <main id="main-content">
      ...
      {page === 'Downloads' && <Downloads jobs={jobs} provider={provider} onJobs={setJobs} onNew={() => { setAnalysis(undefined); setUrl(''); setPage('Início'); }} />}
      {page === 'Histórico' && <SimplePage title="Histórico" subtitle="Mantido somente neste navegador.">{jobs.length ? jobs.map((job) => <JobRow key={job.id} job={job} provider={provider} />) : <Empty text="Nenhum download registrado neste navegador." />}</SimplePage>}
...
function JobRow({ job, provider }: { job: DownloadJob; provider: DownloadProvider }) { const percent = job.progress.percent; return <article className="job-row"><div><span className={`state state-${job.state.toLowerCase()}`}>{job.state}</span><h3>{job.progress.title || job.result?.title || job.id}</h3><p>{job.error || [job.progress.speed, job.progress.eta && `ETA ${job.progress.eta}`, job.progress.size || formatBytes(job.result?.size)].filter(Boolean).join(' • ') || modeLabel(job.mode)}</p></div><div className="job-progress"><progress value={typeof percent === 'number' ? percent : undefined} max="100" aria-label={`Progresso de ${job.progress.title || job.id}`} /><span>{typeof percent === 'number' ? `${percent.toFixed(1)}%` : job.state}</span></div>{job.state === 'COMPLETED' && <a className="primary small" href={provider.fileUrl(job.id)}>Baixar arquivo</a>}</article>; }
function Downloads({ jobs, provider, onJobs, onNew }: { jobs: DownloadJob[]; provider: DownloadProvider; onJobs: React.Dispatch<React.SetStateAction<DownloadJob[]>>; onNew: () => void }) { return <SimplePage title="Downloads" subtitle="Progresso informado pelo yt-dlp e pelo conversor."><div className="page-actions"><button className="primary" onClick={onNew}>Novo download</button></div>{jobs.length ? jobs.map((job) => <div key={job.id} className="job-wrap"><JobRow job={job} provider={provider} />{!terminalStates.has(job.state) && <button className="text-button" onClick={async () => { await provider.cancel(job.id); onJobs((current) => current.map((item) => item.id === job.id ? { ...item, state: 'CANCELLED', progress: { ...item.progress, state: 'CANCELLED' } } : item)); }}>Cancelar</button>}</div>) : <Empty text="Seus downloads aparecerão aqui." />}</SimplePage>; }
