...
import type { Analysis, DownloadJob, DownloadProvider, DownloadRequest, EngineDetection, EngineDetectionState, ProcessingMode, Quality, Settings, ToolStatus } from './contracts';
...
const onlineLimitationCodes = new Set(['YOUTUBE_BOT_CHALLENGE', 'YOUTUBE_PO_TOKEN_REQUIRED', 'YOUTUBE_RATE_LIMITED', 'UPSTREAM_TIMEOUT']);
...
function providerMessage(error: ProviderError) {
  const messages: Record<string, string> = {
    YOUTUBE_BOT_CHALLENGE: 'O YouTube solicitou uma verificação antibot ao servidor online.',
    YOUTUBE_PO_TOKEN_REQUIRED: 'O YouTube exigiu uma prova de origem que o servidor online não possui.',
    YOUTUBE_RATE_LIMITED: 'O modo online está temporariamente limitado pelo YouTube.',
    YOUTUBE_UNAVAILABLE: 'Este conteúdo não está disponível para processamento.',
    UPSTREAM_TIMEOUT: 'O YouTube demorou demais para responder. Tente novamente em instantes.',
    EXTRACTOR_FAILED: 'O extrator não conseguiu processar este conteúdo.',
    ENGINE_AUTH_REQUIRED: 'Código de conexão inválido.'
  };
  return messages[error.code] || error.message;
}
function initialEngineToken() {
  const token = new URLSearchParams(location.hash.slice(1)).get('token') || '';
  if (token) { localStorage.setItem('mp3-engine-token', token); history.replaceState(null, '', location.pathname + location.search); return token; }
  return localStorage.getItem('mp3-engine-token') || '';
}
...
export default function App() {
  ...
  const [engineState, setEngineState] = useState<EngineDetectionState>(desktop ? 'READY' : 'NOT_DETECTED');
  const [engineBaseUrl, setEngineBaseUrl] = useState('http://127.0.0.1:38765');
  const [enginePaired, setEnginePaired] = useState(desktop && Boolean(engineToken));
  const [engineConnecting, setEngineConnecting] = useState(false);
  ...
  const engineAvailable = engineState === 'READY' && enginePaired;
  const engineReachable = !['NOT_DETECTED', 'PERMISSION_REQUIRED'].includes(engineState);
  const provider = useMemo<DownloadProvider>(() => mode === 'WEB_CLOUD' ? cloudProvider() : localProvider(engineToken, desktop, engineBaseUrl), [mode, engineToken, desktop, engineBaseUrl]);
  const providerForMode = useCallback((jobMode: ProcessingMode): DownloadProvider => jobMode === 'WEB_CLOUD' ? cloudProvider() : localProvider(engineToken, jobMode === 'DESKTOP_LOCAL' && desktop, engineBaseUrl), [engineToken, desktop, engineBaseUrl]);

  const applyDetection = useCallback((result: EngineDetection) => {
    setEngineState(result.state); setEngineVersion(result.version); setTools(result.tools || {});
    if (result.reachable && result.baseUrl) setEngineBaseUrl(result.baseUrl);
    if (result.state !== 'READY') setEnginePaired(false);
  }, []);

  useEffect(() => {
    if (desktop) return;
    detectLocalEngine().then(applyDetection);
  }, [desktop, applyDetection]);
  useEffect(() => { localStorage.setItem('mp3-default-quality', quality); }, [quality]);
  useEffect(() => { if (mode !== 'WEB_CLOUD' && enginePaired) provider.getSettings().then((value) => { setSettings(value); setQuality(value.defaultQuality); }).catch(() => undefined); }, [mode, enginePaired, provider]);
  ...
  async function analyze(event: FormEvent) {
    event.preventDefault(); await runAnalysis();
  }

  async function runAnalysis() {
    setError(''); setErrorCode(''); setAnalysis(undefined); setBusy(true);
    ...
    catch (reason) { if (reason instanceof ProviderError) { setError(providerMessage(reason)); setErrorCode(reason.code); } else setError('Não foi possível analisar o link agora.'); }
    finally { setBusy(false); }
  }

  async function startDownload() {
    setError(''); setErrorCode(''); setBusy(true);
    ...
    catch (reason) { if (reason instanceof ProviderError) { setError(providerMessage(reason)); setErrorCode(reason.code); } else setError('Não foi possível iniciar o download.'); }
    finally { setBusy(false); }
  }

  async function connectEngine() {
    setEngineConnecting(true); setConnectionAttempted(true); setError(''); setErrorCode('');
    const result = desktop ? { state: 'READY', available: true, reachable: true, baseUrl: '' } as EngineDetection : await detectLocalEngine({ interactive: true, attempts: 2, timeoutMs: 1200 });
    applyDetection(result);
    if (result.state === 'READY') {
      if (!engineToken) { setEngineState('AUTH_REQUIRED'); setError('Informe o código de conexão exibido pelo Engine.'); setPage('Configurações'); }
      else await validateEngineToken(result.baseUrl || engineBaseUrl);
    } else if (result.state === 'PERMISSION_REQUIRED') setError('Autorize este site a acessar a rede local e tente novamente.');
    else if (result.state === 'TOOLS_NOT_READY') setError('Engine encontrado, mas a preparação das ferramentas está incompleta.');
    else setError('Para usar o processamento local, instale ou abra o MP3 Downloader para Windows.');
    setEngineConnecting(false);
  }

  async function validateEngineToken(baseUrl = engineBaseUrl) {
    if (!engineToken) { setEngineState('AUTH_REQUIRED'); setEnginePaired(false); setError('Informe o código de conexão exibido pelo Engine.'); return; }
    try {
      const value = await localProvider(engineToken, desktop, baseUrl).getSettings();
      localStorage.setItem('mp3-engine-token', engineToken); setSettings(value); setQuality(value.defaultQuality);
      setEngineState('READY'); setEnginePaired(true); setError(''); setErrorCode(''); setMode(desktop ? 'DESKTOP_LOCAL' : 'LOCAL_ENGINE');
    } catch (reason) {
      localStorage.removeItem('mp3-engine-token'); setEnginePaired(false); setEngineState('AUTH_REQUIRED');
      setError(reason instanceof ProviderError && reason.code === 'ENGINE_AUTH_REQUIRED' ? 'Código de conexão inválido.' : 'Não foi possível validar o Engine local.');
      setPage('Configurações');
    }
  }

  function chooseLocal() {
    if (!enginePaired) { void connectEngine(); return; }
  ...
  return <div className="app-shell">
    ...
    <main id="main-content">
      {page === 'Início' && <section className="home">
        ...
        <form className="download-card" onSubmit={analyze}>
          ...
          <div className="provider-row">
            ...
            <article className={mode !== 'WEB_CLOUD' ? 'provider provider-local active' : 'provider provider-local'}>
              <span><MonitorDown size={22} aria-hidden="true" /></span><span><strong>{engineAvailable ? 'Processamento local disponível' : engineState === 'TOOLS_NOT_READY' ? 'Engine encontrado — ferramentas incompletas' : engineState === 'PERMISSION_REQUIRED' ? 'Permissão de rede local necessária' : engineState === 'AUTH_REQUIRED' ? 'Engine encontrado — conexão necessária' : engineReachable ? 'Engine encontrado' : 'Engine não conectado'}</strong><small>{engineVersion ? `Engine ${engineVersion}` : engineState === 'NOT_DETECTED' ? 'Instale ou abra o aplicativo Windows' : 'Mais privado e direto'}</small></span><i>{mode !== 'WEB_CLOUD' ? 'Selecionado' : engineAvailable ? 'Disponível' : engineState}</i>
              <div className="provider-actions">{engineAvailable ? <button type="button" className="primary small" onClick={chooseLocal}>Selecionar</button> : <button type="button" className="primary small" onClick={() => void connectEngine()} disabled={engineConnecting}>{engineConnecting ? 'Conectando…' : engineState === 'NOT_DETECTED' && connectionAttempted ? 'Já abri o Engine — tentar novamente' : ['NOT_DETECTED', 'READY', 'AUTH_REQUIRED'].includes(engineState) ? 'Conectar Engine' : 'Tentar novamente'}</button>}<a className="secondary-link" href="https://github.com/misaeldasilva123ms96-commits/BAIXAR-MP3/releases/latest">Baixar Engine para Windows</a></div>
            </article>
          </div>
          {error && <div className="notice error" role="alert"><span>{error}</span>{mode === 'WEB_CLOUD' && onlineLimitationCodes.has(errorCode) && <div className="notice-actions"><button type="button" className="primary small" onClick={() => void runAnalysis()}>Tentar novamente</button><button type="button" className="secondary" onClick={() => void connectEngine()}>Usar processamento local</button></div>}</div>}
        </form>
      ...
      {page === 'Configurações' && <SimplePage title="Configurações" subtitle="Preferências deste dispositivo."><div className="settings-card"><label>Qualidade padrão<select value={settings.defaultQuality} onChange={(e) => setSettings((value) => ({ ...value, defaultQuality: e.target.value as Quality }))}>{qualityOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>{mode !== 'WEB_CLOUD' && <label>Pasta de downloads<input value={settings.downloadDirectory} onChange={(e) => setSettings((value) => ({ ...value, downloadDirectory: e.target.value }))} placeholder="C:\Users\...\Downloads\Musicas_MP3" /></label>}<label className="check"><input type="checkbox" checked={settings.organizePlaylist} onChange={(e) => setSettings((value) => ({ ...value, organizePlaylist: e.target.checked }))} /> Criar pasta para playlists</label><label className="check"><input type="checkbox" checked={settings.avoidDuplicates} onChange={(e) => setSettings((value) => ({ ...value, avoidDuplicates: e.target.checked }))} /> Evitar downloads repetidos no Windows</label><label className="check"><input type="checkbox" checked={settings.embedThumbnail} onChange={(e) => setSettings((value) => ({ ...value, embedThumbnail: e.target.checked }))} /> Incorporar thumbnail</label><label className="check"><input type="checkbox" checked={settings.embedMetadata} onChange={(e) => setSettings((value) => ({ ...value, embedMetadata: e.target.checked }))} /> Incorporar metadados</label><label>Código de conexão do Engine<input type="password" value={engineToken} onChange={(e) => { setEngineToken(e.target.value.trim()); setEnginePaired(false); }} placeholder="Cole o código exibido pelo Engine" /></label><div className="settings-actions"><button className="primary" onClick={() => void connectEngine()}>Testar conexão</button><span aria-live="polite">{enginePaired ? '✓ Engine conectado' : engineState === 'AUTH_REQUIRED' && error ? error : ''}</span></div><button className="primary" onClick={async () => { localStorage.setItem('mp3-default-quality', settings.defaultQuality); setQuality(settings.defaultQuality); if (enginePaired) { try { await localProvider(engineToken, desktop, engineBaseUrl).saveSettings(settings); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Não foi possível salvar.'); return; } } }}>Salvar configurações</button><p>O código impede que outros sites controlem o Engine apenas por ele estar em localhost. Ele nunca é enviado ao modo online.</p></div>{error && <div className="notice error" role="alert">{error}</div>}</SimplePage>}
      {page === 'Ferramentas' && <SimplePage title="Ferramentas" subtitle="Estado informado pelo runtime, sem estimativas."><div className="settings-card"><p><strong>Engine local:</strong> {engineAvailable ? `Pronto${engineVersion ? ` — ${engineVersion}` : ''}` : engineReachable || desktop ? 'Encontrado, mas ainda não pronto' : 'Não detectado'}</p>{Object.entries(tools).map(([name, value]) => <p key={name}><strong>{name}:</strong> {value}</p>)}<button className="primary" onClick={() => void connectEngine()}>Verificar ferramentas</button> <a className="primary tool-link" href="https://github.com/misaeldasilva123ms96-commits/BAIXAR-MP3/releases/latest">Atualizar ferramentas</a><p>O Engine valida yt-dlp, FFmpeg, ffprobe, Deno e EJS antes de declarar prontidão.</p></div></SimplePage>}
      {page === 'Sobre' && <SimplePage title="Sobre" subtitle="Um produto, uma interface, múltiplos modos."><div className="settings-card"><p>MP3 Downloader v3.0.1</p><p>Use apenas conteúdo próprio, licenciado, em domínio público ou para o qual você tenha autorização. Respeite direitos autorais e os termos da plataforma.</p></div></SimplePage>}
    </main>
...
function JobRow({ job, provider, onError }: { job: DownloadJob; provider: DownloadProvider; onError: (message: string) => void }) { const percent = job.progress.percent; const jobError = job.errorCode ? providerMessage(new ProviderError(job.error || '', job.errorCode)) : job.error; return <article className="job-row"><div><span className={`state state-${job.state.toLowerCase()}`}>{job.state}</span><h3>{job.progress.title || job.result?.title || job.id}</h3><p>{jobError || [job.progress.speed, job.progress.eta && `ETA ${job.progress.eta}`, job.progress.size || formatBytes(job.result?.size)].filter(Boolean).join(' • ') || modeLabel(job.mode)}</p></div><div className="job-progress"><progress value={typeof percent === 'number' ? percent : undefined} max="100" aria-label={`Progresso de ${job.progress.title || job.id}`} /><span>{typeof percent === 'number' ? `${percent.toFixed(1)}%` : job.state}</span></div>{job.state === 'COMPLETED' && <button className="primary small" onClick={() => provider.downloadFile(job.id).catch((reason) => onError(reason instanceof Error ? reason.message : 'Não foi possível baixar o arquivo.'))}>Baixar arquivo</button>}</article>; }
