...
func main() {
	...
	processor := &core.ExecProcessor{Tools: tools, OutputRoot: filepath.Join(root, "jobs"), TempRoot: filepath.Join(root, "temp"), Cloud: true, MaxItems: runtimeconfig.EnvInt("MP3_MAX_PLAYLIST_ITEMS", 100), MaxOutputBytes: int64(runtimeconfig.EnvInt("MP3_MAX_OUTPUT_MB", 500)) * 1024 * 1024, PlayerClients: runtimeconfig.Env("MP3_YOUTUBE_PLAYER_CLIENTS", "")}
